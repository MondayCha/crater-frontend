...
const refreshTokenFn = async (): Promise<string> => {
  ...
  // 使用基本的 ky 实例避免循环调用
  const baseUrl = store.get(configUrlApiBaseAtom)
...
export const apiPublicGet = <T>(url: string, options?: Options) =>
  apiRequest(() => apiClient.get(url, options).json<T>())

export const apiPublicPost = <T>(url: string, json?: unknown) =>
  apiRequest(() => apiClient.post(url, { json }).json<T>())

export const apiPublicPut = <T>(url: string, json?: unknown) =>
  apiRequest(() => apiClient.put(url, { json }).json<T>())

export const apiPublicDelete = <T>(url: string, json?: unknown) =>
